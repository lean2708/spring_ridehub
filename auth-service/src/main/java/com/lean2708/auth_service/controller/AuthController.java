...
public class AuthController {
    ...
    @Operation(summary = "Gửi OTP đăng ký",
            description = "Bước 1 đăng kí: API gửi mã OTP tới số điện thoại người dùng để bắt đầu quá trình đăng ký")
    ...
    @Operation(summary = "Xác thực OTP đăng ký",
            description = "Bước 2 đăng kí: API xác thực mã OTP đã gửi tới email trong quá trình đăng ký.")
    @PostMapping("/register/verify")
    public ApiResponse<VerifyOtpResponse> verifyRegistrationOtp(@Valid @RequestBody VerifyOtpRequest request) {
        log.info("Received request verifying OTP for phone: {}", request.getPhone());
    ...
    @Operation(summary = "Thêm thông tin cá nhân",
            description = "Bước 3 đăng kí: API lưu tạm thông tin cá nhân của người dùng (họ tên, số điện thoại) trước khi thiết lập mật khẩu.")
    ...
    public ApiResponse<UserDetailsResponse> addUserDetails(@Valid @RequestBody RegisterDetailsRequest request) {
        log.info("Received adding user details for phone: {}", request.getPhone());
    ...
    @Operation(summary = "Đặt mật khẩu và hoàn tất đăng ký",
            description = "Bước 4 đăng kí: API tạo tài khoản người dùng và thiết lập mật khẩu dựa trên thông tin đã xác thực trước đó.")
    ...
    public ApiResponse<TokenResponse> setPassword(@Valid @RequestBody SetPasswordRequest request) throws JOSEException {
        log.info("Received setting password and creating user for phone: {}", request.getPhone());
    ...
    @Operation(summary = "Quên mật khẩu",
            description = "Bước 1 quên mk: API gửi mã OTP tới email để bắt đầu quá trình khôi phục mật khẩu.")
    ...
    @Operation(summary = "Xác thực mã OTP quên mật khẩu",
            description = "Bước 2 quên mk: API xác thực mã OTP trong quá trình khôi phục mật khẩu.")
    ...
    @Operation(summary = "Đặt lại mật khẩu",
            description = "Bước 3 quên mk: API đặt lại mật khẩu mới cho người dùng sau khi xác thực OTP thành công.")
